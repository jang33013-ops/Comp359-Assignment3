"""
Tests for generate_trees() and get_max_degree()
Run with:  python test_generate_trees.py
"""

from generate_trees import generate_trees, get_max_degree


# Tests for get_max_degree()
...
def test_max_degree():
    ...
    tests = [
        # (edges, n, expected_max_degree, description)
...
def test_generate_trees():
    ...
    #  n=2: single edge, no Prüfer code 
    accepted, stopped, checked = generate_trees(2)
    ...
    #  n=3: 3 trees total, all have max degree <= 3 
    accepted, stopped, checked = generate_trees(3)
    ...
    #  n=4: 4^2=16 Prüfer sequences, all trees have max degree <= 3 
    accepted, stopped, checked = generate_trees(4)
    ...
    #  n=4, max_degree=1: no tree on 4 vertices has max degree 1 
    accepted, stopped, checked = generate_trees(4, max_degree=1)
    check("n=4 max_degree=1 → 0 accepted", len(accepted) == 0)

    #  n=4, max_degree=2: only paths allowed 
    accepted, stopped, checked = generate_trees(4, max_degree=2)
    check("n=4 max_degree=2 all have degree<=2",
          all(get_max_degree(e, 4) <= 2 for e in accepted))

    #  stop_at=3 on n=4 
    accepted, stopped, checked = generate_trees(4, stop_at=3)
    check("stop_at=3 returns exactly 3",  len(accepted) == 3)
    check("stop_at=3 stopped_early True", stopped == True)

    #  n=7 main case: 100 trees
    accepted, stopped, checked = generate_trees(7, max_degree=3, stop_at=100)
