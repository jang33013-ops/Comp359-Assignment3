...
![comparison.py final output](comparison_result.jpg)
...
![decode_prufer_verbose output](decode_prufer_verbose.png)
...
The verbose trace above (screenshot) was used to trace `[1, 1, 3]` step by step. At step 2, the remaining code is `[1, 3]` and the available set is `[1, 3, 4, 5]`. The smallest label **not** in the code is `4` — not `3` — because `3` still appears in the remaining code. The algorithm was producing the correct output all along.
...
![test_decode_prufer.py all passing](test_decode_prufer_passing.png)
...
![encode_pruferM4.py all passing](encode_prufer_passing.png)
...
**Note:** Test 3's code came out as `[6, 1, 2, 6]` (not `[2, 6, 6, 1]` as written earlier) — the encode output is the correct one since no expected code was provided for that test, and the round-trip still matched.
...
![test_generate_trees.py all passing](test_generate_trees_passing.png)
